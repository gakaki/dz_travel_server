...
class WeChatController extends Controller {
    async auth(ctx){
        ...
        let resultS = await this.service.weChatService.weChat.auth(sdkAuth);
        if(resultS!=null){
            result.code=0;
    ...
    async minapppay(ctx){
        ...
        ctx.body=await this.service.weChatService.weChat.minAppPay(ui,payCount,title,appName);
    }
    async minappwithdraw(ctx){
        ...
        let count=await ctx.model.WeChatModel.WechatPaytoUser.count({"openid":ui.uid,"created":new Date().toLocaleDateString(),"success":true,appName:appName});
        ...
        let res=await this.service.weChatService.weChat.minAppWithdraw(ui,money,appName);
        if(res){
    ...
    async shopdone(ctx){
        ...
        this.service.weChatService.weChat.shopDone(appName);
