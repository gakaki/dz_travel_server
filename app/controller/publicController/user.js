...
class UserController extends Controller {
    ...
    async login(ctx) {
        ...
        let rs=await this.service.publicService.user.login(ctx.query);
        if(rs.info !=null){
            result.code=0;
    ...
    async getiteminfo(ctx){
        ...
        let ui=await this.service.publicService.user.findUserBySid(ctx.query._sid);
        if(ui==null){
    ...
    async changeitem(ctx){
        ...
        await this.ctx.service.publicService.item.itemChange(ui,cost,appName);
    }
