...
class StrategyService extends Service {
   async getTravelStrategy(info) {
       ...
       for(let travel_tip of travelTips) {
           ...
           if(info.type == apis.PostType.JINGDIAN) {
               ...
               tsinfo.img = travelConfig.Scenicspot.Get(travel_tip).picture;
   ...
   async giveThumbsUp(info, comment) {
       ...
       if(count * travelConfig.Parameter.Get(travelConfig.Parameter.THUMBUPGOLD).value < travelConfig.Parameter.Get(travelConfig.Parameter.THUMLIMIT).value) {
           //被点赞的人获得金币
           this.ctx.service.publicService.itemService.itemChange(comment.uid, { ["items." + travelConfig.Item.GOLD]: travelConfig.Parameter.Get(travelConfig.Parameter.THUMBUPGOLD).value }, "like");
           //通知被赞人
