...
class GuessnumService extends Service {
    async sendPack(ui, money, title, useTicket, orderid = null) {
        ...
        let cost = {
            ["items." + guessnumConfigs.Item.ACCELERATION]: 2
        };
        if (useTicket) {
            cost  ["items." + guessnumConfigs.Item.CASHCOUPON] = -1;
            money = 1;
            if (ui.items[guessnumConfigs.Item.CASHCOUPON] <= 0) {
        ...
        let packInfo = await this.ctx.model.GuessnumModel.PackInfo.create(pack);
        this.logger.info("生成的红包信息 " + JSON.stringify(packInfo));
        packInfo.userInfo = ui;
        let that = this;
        setTimeout(async function () {
            let p = await that.ctx.model.GuessnumModel.PackInfo.findOne({pid: packInfo.pid});
            that.logger.info("红包要过期了 :" + JSON.stringify(p));
            p.status = constant.Code.PACK_EXPIRED;
            await that.ctx.model.GuessnumModel.PackInfo.update({pid: p.pid}, {$set: {status: constant.Code.PACK_EXPIRED}});
            if (!useTicket) {
                let recordsCount = await  that.ctx.model.GuessnumModel.PackGuessRecord.count({pid: p.pid});
                if (recordsCount > 0) {
        ...
        }, Number(guessnumConfigs.Parameter.Get("expire").value) * 3600 * 1000);
    ...
    async guessPack(ui, pack, guessNum, sid) {
        ...
        if (time.getTime() + Number(guessnumConfigs.Parameter.Get("expire").value) * 60 * 60 * 1000 <= new Date().getTime()) {
            result.code = constant.Code.PACK_EXPIRED;
            pack.status = constant.Code.PACK_EXPIRED;
            this.ctx.model.GuessnumModel.PackInfo.update({pid: pack.pid}, {$set: {status: constant.Code.PACK_EXPIRED}});
            return result;
        }
        ...
        if (pack.guessCount <= 0) {
            ...
            this.ctx.model.GuessnumModel.PackInfo.update({pid: pack.pid}, {$set: {status: constant.Code.COUNT_OVER}});
            return result;
        }


        if (pack.CDList[sid]) {
            if (pack.CDList[sid] + Number(guessnumConfigs.Parameter.Get("waitcd").value) * 1000 >= new Date().getTime()) {
                result.code = constant.Code.PACK_ISCD;
                result.data.restTime = new Date(pack.CDList[sid] + Number(guessnumConfigs.Parameter.Get("waitcd").value) * 1000 - new Date().getTime()).toTimeString();
                return result;
            } else {
        ...
        switch (A + B) {
            case 4:
                let cfgAAAA = guessnumConfigs.Distribution.Get(4);
                if (pack.AAAA) {
                ...
                break;
            case 3:
                let cfgAAA = guessnumConfigs.Distribution.Get(3);
                if (pack.AAA) {
                ...
                break;
            case 2:
                let cfgAA = guessnumConfigs.Distribution.Get(2);
                if (pack.AA) {
                ...
                break;
            case 1:
                let cfgA = guessnumConfigs.Distribution.Get(1);
                if (pack.A) {
                ...
                break;
            case 0:
                let cfg = guessnumConfigs.Distribution.Get(5);
                if (pack.miss) {
        ...
        switch (cid) {
            case 1:
                result.data.commit = guessnumConfigs.Evaluate.Get(Number(result.data.markId)).iqwored1;
                break;
            case 2:
                result.data.commit = guessnumConfigs.Evaluate.Get(Number(result.data.markId)).iqwored2;
                break;
            case 3:
                result.data.commit = guessnumConfigs.Evaluate.Get(Number(result.data.markId)).iqwored3;
                break;
        }
        ...
        await this.ctx.model.GuessnumModel.PackInfo.update({pid: pack.pid}, pack);
    ...
    async clearCD(ui, pack, sid) {
        ...
        this.logger.info("剩余加速卡:" + ui.items[guessnumConfigs.Item.ACCELERATION]);

        if (ui.items[guessnumConfigs.Item.ACCELERATION] <= 0) {
        ...
        if (pack.CDList[sid]) {
            if (pack.CDList[sid] + Number(guessnumConfigs.Parameter.Get("waitcd").value) * 1000 >= new Date().getTime()) {
                await this.ctx.model.GuessnumModel.PackInfo.update({pid: pack.pid}, {$unset: {["CDList." + sid]: 1}});
                result.code = constant.Code.OK;
            } else {
    ...
    async getPackRankingList(ui, pack) {
        ...
        let r = await this.ctx.model.GuessnumModel.PackGuessRecord.aggregate([{$match: {pid: pack.pid,}}]).group({
            _id: "$uid",
            moneyGot: {$sum: "$userGetMoney"}
        }).sort({moneyGot: -1});
    ...
    async getUserPackRecords(ui, sendPage, sendLimit, receivePage, receiveLimit) {
        ...
        sendPackage.num = await this.ctx.model.GuessnumModel.PackInfo.count({uid: ui.uid});
        sendPackage.record = await this.ctx.model.GuessnumModel.PackInfo.find({uid: ui.uid}).limit(sendLimit).skip((sendPage - 1) * sendLimit).sort({"createTime": -1});
        ...
        receivePackage.num = await this.ctx.model.GuessnumModel.PackGuessRecord.count({uid: ui.uid});
    ...
    async getAcceleration(ui) {
        ...
        if (count < 1) {
            ...
            userShareRecord.itemId = guessnumConfigs.Item.ACCELERATION;
            result.code = constant.Code.OK;
        } else {
    ...
    getMarkId(mark) {
        for (let i of guessnumConfigs.evaluates) {
            if (i[1] == mark) {
                return i[0];
            }
        }
    ...
    async getPackSumByUid(uid) {
        let r = await this.ctx.model.GuessnumModel.PackInfo.aggregate([{$match: {uid: uid,}}]).group({
            _id: "$uid",
            sum: {$sum: "$money"}
        }).limit(1);
    ...
    async getReceivePackageRecordsMoneyByUid(uid) {
        let r = await this.ctx.model.GuessnumModel.PackGuessRecord.aggregate([{
            $match: {uid: uid,}
        }]).group({_id: "$uid", moneyGot: {$sum: "$userGetMoney"}}).limit(1);
    ...
    async getReceivePackageRecordsByUid(uid, limit, skip, sort) {
        let ps = await this.ctx.model.GuessnumModel.PackGuessRecord.find({uid: uid}).limit(limit).skip(skip).sort(sort);
        let getPacks = [];
        for (let p of ps) {
            let getPack = {};
